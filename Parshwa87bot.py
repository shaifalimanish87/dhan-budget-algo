...
# Ticker mapping for yfinance fallback
MONITOR_INDICES = {
    "NIFTY": {
        "security_id": "13", 
        "yf_symbol": "^NSEI", 
...
def get_live_data_yfinance(symbol):
    """Guaranteed Live Spot Price and 5m Candle Fetcher"""
    try:
        ...
        if not df.empty:
            ...
            return spot_price, rsi_5m, df
    except Exception as e:
        print(f"yfinance error for {symbol}: {e}")
    return 0.0, 50.0, pd.DataFrame()

def fetch_option_chain_data(security_id):
    """Fetch Dhan Option Chain safely"""
    try:
...
def run_trading_engine():
    print("🚀 PRO-MODE: Auto-Trigger High-Speed Scan Started...")
    ...
    for index_name, info in MONITOR_INDICES.items():
        ...
        # Fetch Spot Price & RSI via yfinance (100% reliable)
        current_spot, rsi_5m, df_5m = get_live_data_yfinance(yf_symbol)
        
        # Fetch PCR & Chain via Dhan API
